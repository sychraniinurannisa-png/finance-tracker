...
export function Income() {
  ...
  // Reset page ke 1 saat filter/pencarian berubah
  useEffect(() => {
    setCurrentPage(1);
  ...
  const fetchIncomes = async () => {
    try {
      ...
      triggerAlert("error", "Gagal memuat data pemasukan: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!amount || parseFloat(amount) <= 0) {
      triggerAlert("error", "Nominal pemasukan harus lebih besar dari 0");
      return;
    }
    ...
    try {
      ...
      if (modalMode === "add") {
        const { error } = await supabase
          ...
          .insert([payload]);
        if (error) throw error;
        triggerAlert("success", "Pemasukan berhasil ditambahkan!");
      } else {
        const { error } = await supabase
          .from("transactions")
          .update({
        ...
        if (error) throw error;
        triggerAlert("success", "Pemasukan berhasil diubah!");
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) throw error;
      triggerAlert("success", "Pemasukan berhasil dihapus!");
  ...
  return (
    <Layout title="Arus Pemasukan" description="Kelola riwayat pendapatan dan arus pemasukan kas keuangan pribadi Anda.">
      {alert.message && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-5 text-white shadow-sm shadow-emerald-500/10 flex flex-col justify-between h-32 group hover:shadow-md hover:shadow-emerald-500/20 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-emerald-100 uppercase tracking-wider">Total Pemasukan</span>
          ...
          <div>
            ...
            <p className="text-[10px] text-emerald-100/80 mt-1">Akumulasi seluruh riwayat pendapatan</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200/60 rounded-2xl p-5 shadow-sm flex flex-col justify-between h-32 hover:border-slate-300 hover:shadow-sm transition-all">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-1">Nilai rata-rata dari pemasukan masuk</p>
          </div>
        </div>

        <div className="bg-white border border-slate-200/60 rounded-2xl p-5 shadow-sm flex flex-col justify-between h-32 hover:border-slate-300 hover:shadow-sm transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Frekuensi Masuk</span>
            <span className="bg-slate-50 text-slate-500 p-2 rounded-xl text-xs">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-1">Jumlah seluruh transaksi tercatat</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200/60 rounded-2xl p-5 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button 
            ...
            className="bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-600 hover:to-teal-500 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition-all shadow-md shadow-emerald-500/10 active:scale-[0.98] flex items-center justify-center gap-2 self-start md:self-auto"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 border-t border-slate-100 pt-4">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-50/60 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-emerald-500 transition-all placeholder:text-slate-400 text-slate-700"
            />
          </div>
          ...
          <div className="relative">
            <select
              ...
              className="w-full px-3 py-2 bg-slate-50/60 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-emerald-500 transition-all text-slate-650 appearance-none"
            >
          ...
          {loading ? (
            ...
            <div className="py-12 flex flex-col items-center justify-center text-center space-y-4">
              ...
              <div className="space-y-1">
                <h5 className="text-xs font-bold text-slate-700">Tidak Ada Data Pemasukan</h5>
                <p className="text-[10px] text-slate-400 max-w-[280px]">
                  {searchTerm || selectedCategory 
                    ? "Coba ganti filter atau kata pencarian Anda." 
                    : "Mulai dengan menambahkan pemasukan pertama Anda untuk mencatat riwayat keuangan."}
                </p>
              </div>
              {!searchTerm && !selectedCategory && (
                <button
                  ...
                  >
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <h5 className="text-xs font-bold text-slate-800 truncate">
                            {item.description || "Pemasukan Uang"}
                          </h5>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <span className="text-xs font-black text-emerald-600">+{formatRupiah(item.amount)}</span>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-slate-100 mt-4">
              <span className="text-[11px] text-slate-500 font-medium">
                ...
                <span className="font-bold text-slate-700">
                  {Math.min(startIndex + ITEMS_PER_PAGE, filteredIncomes.length)}
                </span>{" "}
                dari <span className="font-bold text-slate-700">{filteredIncomes.length}</span> data
              </span>
            ...
            <div className="space-y-4">
              <div>
                <h3 className="text-base font-extrabold text-slate-900">
                  {modalMode === "add" ? "Tambah Pemasukan Baru" : "Ubah Data Pemasukan"}
                </h3>
                <p className="text-[11px] text-slate-400 mt-0.5">
                  {modalMode === "add" 
                    ? "Isi formulir berikut untuk menambahkan riwayat pemasukan uang Anda." 
                    : "Perbarui informasi transaksi pendapatan terpilih di bawah ini."}
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 pt-2">
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      disabled={submitLoading}
                      className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500 transition-all font-bold text-slate-700"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center">
                    ...
                    <a href="/kategori" className="text-[9px] font-bold text-emerald-600 hover:text-emerald-700">
                  ...
                  <div className="relative">
                    <select 
                      ...
                      disabled={submitLoading}
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500 transition-all text-slate-650 appearance-none"
                    >
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input 
                      ...
                      disabled={submitLoading}
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500 transition-all text-slate-700"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea 
                    id="description" 
                    placeholder="Contoh: Pembayaran upah freelance desain grafis logo perusahaan"
                    value={description}
                    rows="3"
                    onChange={(e) => setDescription(e.target.value)}
                    disabled={submitLoading}
                    maxLength="250"
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-500 transition-all text-slate-700 placeholder:text-slate-400 resize-none"
                  ></textarea>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button 
                    type="submit"
                    disabled={submitLoading}
                    className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-600 hover:to-teal-500 text-white font-bold py-3.5 rounded-xl text-xs transition-all shadow-md shadow-emerald-500/10 active:scale-[0.98] flex items-center justify-center gap-2"
                  >
                    {submitLoading ? (
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl border border-slate-100 text-center space-y-5">
            ...
            <div className="space-y-1">
              <h3 className="text-sm font-extrabold text-slate-800">Hapus Catatan Pemasukan?</h3>
              <p className="text-[10px] text-slate-400 max-w-[250px] mx-auto">
                Tindakan ini tidak dapat dibatalkan. Riwayat transaksi "{transactionToDelete?.description || "Pemasukan"}" sebesar <span className="font-bold text-rose-600">{formatRupiah(transactionToDelete?.amount || 0)}</span> akan terhapus permanen.
              </p>
            </div>
